...
namespace Pixi
{
  ...
  PixiHostObject::PixiHostObject(jsi::Runtime &runtime)
  {
  }
  ...
  jsi::Value PixiHostObject::get(jsi::Runtime &runtime, const jsi::PropNameID &propNameId)
  {
    ...
    if (propName == "convert")
    {
      ...
      return jsi::Function::createFromHostFunction(runtime,
                                                   ...
                                                   {
                                                     std::shared_ptr hostObject = arguments[0].getObject(runtime).asHostObject(runtime);
                                                     jsi::Object optionsObject = arguments[1].getObject(runtime);
                                                     int output_width = optionsObject.getProperty(runtime, "output_width").asNumber();
                                                     int output_height = optionsObject.getProperty(runtime, "output_height").asNumber();
                                                     std::string output_format = optionsObject.getProperty(runtime, "output_format").getString(runtime).utf8(runtime);

                                                     if (output_format != "rgba-8888")
                                                     {
                                                       throw jsi::JSError(runtime, "Unsupported output format: " + output_format + ". Supported formats are: rgba-8888");
                                                     }
                                                     ...
                                                     if (frame != nullptr)
                                                     {
                                                       ...
                                                       AHardwareBuffer_describe(ahb, &bufferDescription);

                                                       YUVLayout layout = {0};
                                                       AHardwareBuffer_Planes planes;
                                                       AHardwareBuffer_lockPlanes(ahb, AHARDWAREBUFFER_USAGE_CPU_READ_MASK, -1, nullptr, &planes);
                                                       ...
                                                       halide_dimension_t y_dim[] = {{0, frameWidth, 1}, {0, frameHeight, layout.y_row_stride}};
                                                       halide_dimension_t uv_dim[] = {{0, width_uv, 1}, {0, height_uv, layout.uv_row_stride}};

                                                       // 3 - create the Halide::Runtime::Buffer objects
                                                       Halide::Runtime::Buffer<uint8_t> y_buffer(layout.y_data, 2, y_dim);
                                                       Halide::Runtime::Buffer<uint8_t> u_buffer(layout.u_data, 2, uv_dim);
                                                       Halide::Runtime::Buffer<uint8_t> v_buffer(layout.v_data, 2, uv_dim);



                                                       // 4 - create an output buffer with the same dimensions as the Y plane
                                                       std::vector<uint32_t> temp_output(frameWidth * frameHeight);
                                                       Halide::Runtime::Buffer<uint32_t> output_converted(temp_output.data(), frameWidth, frameHeight);
                                                       ...
                                                       int result = yuv_to_rgba888(y_buffer, u_buffer, v_buffer, output_converted);
                                                       auto halide_stop = std::chrono::high_resolution_clock::now();
                                                       auto halide_duration_micro = std::chrono::duration_cast<std::chrono::microseconds>(halide_stop - halide_start);
                                                       double halide_duration_milli = halide_duration_micro.count() / 1000.0;

                                                       __android_log_print(ANDROID_LOG_INFO, "PixiTag",
                                                                             "[yuv420toArgbNative] Execution time: %.3f ms", halide_duration_milli);
                                                       if (result != 0)
                                                       {
                                                         __android_log_print(ANDROID_LOG_ERROR, "PixiTag",
                                                                             "[yuv420toArgbNative] Halide generator execution failed with error code %d.", result);
                                                       }
                                                       ...
                                                       frameDataObj.setProperty(runtime, "output_width", frameWidth);
                                                       frameDataObj.setProperty(runtime, "output_height", frameHeight);
